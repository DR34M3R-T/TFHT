...
data = torch.from_numpy(np.load('./dataset/HIT/data.npy')) #2w 6 2048
label = torch.from_numpy(np.load('./dataset/HIT/'+label_name)) #2w
...
# 实例化dataset
training_data = BearFaultDataset(data_train, label_train, transform=preprocess)
test_data = BearFaultDataset(data_test, lable_test, transform=preprocess)
...
ViT_Channels=data.shape[1]
print("Nunber of ViT channels:{}.".format(ViT_Channels))
v = MyViT_HIT.ViT( #定义ViT模型
    image_size = 2048,
    patch_size = 64,
