...
class BearFaultDataset(Dataset):
    def __init__(self, inputs, targets, transform, reshape):
        ...
        if reshape:
            ...
            self.inputs = torch.cat((torch.unsqueeze(inputs,1),torch.unsqueeze(inputs,1)),1)
...
#ExpLR = decrease_rate_try.Decrease_inregular(optimizer, gamma=0.95, min_lr=learning_rate, max_lr=1e-2, need_iter=2) #绑定衰减学习率到优化器
...
# 定义测试循环
def test_loop(dataloader, model, loss_fn,epoch,epochs,acc):
    size = len(dataloader.dataset)
...
for t in range(epochs): # 开始训练
    ...
    last_loss=now_loss
    now_loss,acc=test_loop(test_dataloader, v, loss_fn,t,epochs,acc)
    # 学习率动态衰减
